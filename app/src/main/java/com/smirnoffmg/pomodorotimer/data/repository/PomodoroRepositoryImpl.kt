...
class PomodoroRepositoryImpl
    ...
    constructor(
        ...
        override suspend fun getSessionById(sessionId: Long): PomodoroSession? = 
            pomodoroSessionDao.getSessionById(sessionId)?.toDomain()
        ...
        override suspend fun getDailyStatistics(date: Long): DailyStatistics {
            ...
            val totalWorkDuration = completedSessions
                .filter { it.type == SessionType.WORK }
                .sumOf { it.duration }
            val totalBreakDuration = completedSessions
        ...
        // Weekly aggregation
        override suspend fun getSessionsByWeek(weekStart: Long, weekEnd: Long): List<PomodoroSession> =
            pomodoroSessionDao.getSessionsByWeek(weekStart, weekEnd).map { it.toDomain() }

        override fun getSessionsByWeekFlow(weekStart: Long, weekEnd: Long): Flow<List<PomodoroSession>> =
            pomodoroSessionDao.getSessionsByWeekFlow(weekStart, weekEnd).map { sessions ->
                sessions.map { it.toDomain() }
            }

        override suspend fun getWeeklyStatistics(weekStart: Long, weekEnd: Long): WeeklyStatistics {
        ...
        // Monthly aggregation
        override suspend fun getSessionsByMonth(monthStart: Long, monthEnd: Long): List<PomodoroSession> =
            pomodoroSessionDao.getSessionsByMonth(monthStart, monthEnd).map { it.toDomain() }

        override fun getSessionsByMonthFlow(monthStart: Long, monthEnd: Long): Flow<List<PomodoroSession>> =
            pomodoroSessionDao.getSessionsByMonthFlow(monthStart, monthEnd).map { sessions ->
                sessions.map { it.toDomain() }
            }

        override suspend fun getMonthlyStatistics(monthStart: Long, monthEnd: Long): MonthlyStatistics {
        ...
        override suspend fun getSessionTypeStatistics(fromDate: Long, sessionType: SessionType): SessionTypeStatistics {
            val sessionTypeString = when (sessionType) {
                SessionType.WORK -> "WORK"
                SessionType.SHORT_BREAK -> "SHORT_BREAK"
                SessionType.LONG_BREAK -> "LONG_BREAK"
            }
            
            val totalCount = pomodoroSessionDao.getCompletedSessionsCountByWeekAndType(
                fromDate, Long.MAX_VALUE, sessionTypeString
            )
            val totalDuration = pomodoroSessionDao.getTotalDurationByWeekAndType(
                fromDate, Long.MAX_VALUE, sessionTypeString
            ) ?: 0L
        ...
        override suspend fun getAverageDurationByType(sessionType: SessionType, fromDate: Long): Double {
            val sessionTypeString = when (sessionType) {
                SessionType.WORK -> "WORK"
                SessionType.SHORT_BREAK -> "SHORT_BREAK"
                SessionType.LONG_BREAK -> "LONG_BREAK"
        ...
        // Data management
        override suspend fun deleteSessionsBeforeDate(beforeDate: Long) =
            pomodoroSessionDao.deleteSessionsBeforeDate(beforeDate)
    }
