...
class StartSessionUseCase @Inject constructor(
    ...
    suspend operator fun invoke(sessionType: SessionType, duration: Long): Long {
...
class GetDailyStatisticsUseCase @Inject constructor(
    ...
    suspend operator fun invoke(date: Long = System.currentTimeMillis()): DailyStatistics {
...
class GetSessionsByDateUseCase @Inject constructor(
    ...
    fun invoke(date: Long): Flow<List<PomodoroSession>> {
        return repository.getSessionsByDateFlow(date)
    }
...
class GetProductivityStreakUseCase @Inject constructor(
    private val repository: PomodoroRepository
) {
    suspend operator fun invoke(sessionType: SessionType = SessionType.WORK): Int {
        ...
        for (dayOffset in 0..30) { // Check up to 30 days back
            ...
            val hasProductiveDay = when (sessionType) {
...
class CleanupOldSessionsUseCase @Inject constructor(
    private val repository: PomodoroRepository
) {
