...
sealed class DatabaseException(message: String, cause: Throwable? = null) : Exception(message, cause) {
    class SessionNotFound(sessionId: Long) : DatabaseException("Session with ID $sessionId not found")
    
    class InvalidSessionData(message: String, cause: Throwable? = null) : 
        DatabaseException("Invalid session data: $message", cause)
    
    class DatabaseCorrupted(message: String, cause: Throwable? = null) : 
        DatabaseException("Database corrupted: $message", cause)
    
    class MigrationFailed(fromVersion: Int, toVersion: Int, cause: Throwable? = null) : 
        DatabaseException("Migration failed from version $fromVersion to $toVersion", cause)
    
    class DiskSpaceInsufficient(cause: Throwable? = null) : 
    ...
    class QueryTimeout(query: String, cause: Throwable? = null) : 
        DatabaseException("Query timed out: $query", cause)
    
    class ConstraintViolation(constraint: String, cause: Throwable? = null) : 
        DatabaseException("Database constraint violation: $constraint", cause)
    
    class UnknownDatabaseError(cause: Throwable) : 
